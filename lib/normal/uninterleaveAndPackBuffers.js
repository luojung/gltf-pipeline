...
function uninterleaveAndPackBuffers(gltf) {
    ...
    delete gltf.packBufferViews_length;
    ...
    for (var accessorId in accessors) {
        ...
        if (accessors.hasOwnProperty(accessorId)) {
            ...
            if (accessor.hasOwnProperty("extras")) {
...
function packAccessorsForTarget(gltf, bufferId, target, accessorsByByteLength, sourceBuffer, packBuffer, packBufferViews, offset) {
    // var bufferViewId = 'bufferView_' + gltf.packBufferViews_length;
    var bufferViewId = gltf.packBufferViews_length;
    ...
    packBufferViews.push(packBufferView);
    gltf.packBufferViews_length++;
    return packOffset;
}

...
function generateNormals(gltf, options) {
    ...
    console.log("generateNormals done !")
...
//判断node中是属于某种类型材料
function hasMaterial(gtlf, node, materialId){
    if (node.hasOwnProperty("mesh")){
        var meshId = node.mesh
        for (var primitiveId in gtlf.meshes[meshId].primitives){
            var primitive = gtlf.meshes[meshId].primitives[primitiveId]
            if (materialId==primitive.material){
                return true
            }
        }
    }
    ...
    if (node.hasOwnProperty("children")){
        for (var childrenId in node.children) {
            let childrenNode = gtlf.nodes[node.children[childrenId]]
            if (hasMaterial(gtlf, childrenNode, materialId)) {
                return true
            }
        }
        ...
        // for (var childrenId in node.children) {
        //     let childrenNode = gtlf.nodes[node.children[childrenId]]
        //     if (childrenNode.hasOwnProperty("mesh")){
        //         var meshId = childrenNode.mesh
        //         for (var primitiveId in gtlf.meshes[meshId].primitives){
        //             var primitive = gtlf.meshes[meshId].primitives[primitiveId]
        //             if (materialId==primitive.material){
        //                 return true
...
//根据材料类型重新组织节点
function refactorNodes(gltf){
    console.log("************refactorNodes******************")
    if (!gltf.nodes.hasOwnProperty(0) || !gltf.nodes[0].children) {
        console.log("lack of node")
        return undefined
    }

    var length = gltf.nodes.length
    var nodes = gltf.nodes
    var childrenIds=gltf.nodes[0].children
    ...
    for(var materialId in gltf.materials){
        ...
        var newNodeIndex = Number(length)+Number(materialId)
        console.log("newNodeIndex", newNodeIndex)
        nodes[newNodeIndex]=newNode

        for (var childrenId=0;childrenId< childrenIds.length; childrenId++) {
            var nodeId = gltf.nodes[0].children[childrenId]
            if (nodes.hasOwnProperty(nodeId)) {
                var node = nodes[nodeId];
                if(hasMaterial(gltf, node, materialId)){
                    //add to material node
                    nodes[newNodeIndex].children.push(gltf.nodes[0].children[childrenId])

                    //del it from root node
                    childrenIds.splice(childrenId, 1)
    ...
    for(var materialId in gltf.materials){
        var newNodeIndex = Number(length)+Number(materialId)
        gltf.nodes[0].children.push(newNodeIndex) 
    }
}
...
function generateMaterial(gltf, primitive, generatedMaterials) {
    ...
    if (!defined(normalParameter)) {
        ...
        generatedMaterialId = gltf.materials.length;
